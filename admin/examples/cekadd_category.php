  ...
  $nama_gambar_baru = $angka_acak.'-'.$category_photo; //menggabungkan angka acak dengan nama file sebenarnya
        if(in_array($ekstensi, $ekstensi_diperbolehkan) == true)  {     
                move_uploaded_file($file_tmp, '../../assets/img/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                  ...
                  $query = "INSERT INTO category_tbl VALUES ('','$category_name', '$nama_gambar_baru', '$category_detail')";
                  ...
                  if(!$result){
                    ...
                    echo "<script>alert('Data berhasil ditambah.');window.location='tbl_category.php';</script>";
                  }

            } else {     
             //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='add_category.php';</script>";
            }
} else {
   $query = "INSERT INTO category_tbl VALUES ('','$category_name', null, '$category_detail')";
                  $result = mysqli_query($conn, $query);
                  // periska query apakah ada error
                  if(!$result){
                      die ("Query gagal dijalankan: ".mysqli_errno($conn).
